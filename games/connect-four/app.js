...
const ROOM_PATH = "connectFourRooms";
...
try {
  ...
  const move = column => runTransaction(roomRef(roomId), current => {
  ...
  function render() {
    ...
    status.textContent = room.status === "waiting" ? "Waiting for opponent" : room.status === "finished" ? (room.winner ? `${playerLabel(room.winner)} wins` : "Draw") : room.turn === mine ? "Your turn" : `${playerLabel(room.turn)} turn`;
