...
try {
  ...
  function render() {
    ...
    boardEl.replaceChildren(...room.board.flatMap((row, r) => row.map((value, c) => { const b = document.createElement("button"); b.className = `cell ${value || ""}`; b.disabled = room.status !== "playing" || room.turn !== mine || value !== null; b.setAttribute("aria-label", `Column ${c + 1}, row ${r + 1}`); b.onclick = () => move(c); return b; })));
  }
